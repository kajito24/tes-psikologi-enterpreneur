...
document.getElementById("testForm").addEventListener("submit", function (e) {
  ...
  document.getElementById(
    "resultText"
  ).innerHTML = `<strong>Skor Anda: ${totalScore}</strong><br>${resultMessage}`;
  ...
  // Kirim data ke vercel KV storage
  const kvRestAPIUrl = `${process.env.KV_REST_API_URL}/set/totalScores`;
  const requestHeaders = {
    Authorization: `Bearer ${process.env.KV_REST_API_TOKEN}`,
    "Content-Type": "application/json",
  };
  const requestBody = JSON.stringify({
    namaLengkap: sessionStorage.getItem("fullName"),
    tahunLahir: sessionStorage.getItem("DOB"),
    skor: totalScore,
  });
  ...
  fetch(kvRestAPIUrl, {
    ...
    .catch((err) =>
      console.error("Error sending data to vercel KV storage", err)
    );
});
